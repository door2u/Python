...
# convert a list of directories to a string for multiple operating systems
# use "$ROOT" to specify hard drive root directory
# TODO:
# windows
def FilePath(fileList):
	retu = ""
	import os
	a = 0
	if len(fileList) > 0 and fileList[0] == "$ROOT":
		# TODO: how to get root directory in windows
		# TODO: should a raw literal be used for windows
		retu += os.sep
		a += 1
	while a < len(fileList):
		retu += fileList[a]
		# TODO: off by one?
		if a < len(fileList) - 1 and fileList[a + 1] != os.sep:
			retu += os.sep
		a += 1
	return retu
...
"""
def SortFull(lis_, reve = False):
	dime = 0
	if type(lis_) == list and len(lis_) > 0:
		lis_ = sorted(lis_, reverse = reve)
		#a = 0
		#if len(lis_) > 1:
		#	while lis_[a] == lis_[a + 1]
		#li__ = lis_[0]

		star = 0
		li__ = []
		for a in range(1, len(lis_)):
			if isinstance(lis_[a]
			li__.append(lis_[0])
		

		dime = 1
		while isinstance(li__, list):
			#dime += 1
			li__ = li__[0]
			a = 0
			if len(lis_) > 1:
