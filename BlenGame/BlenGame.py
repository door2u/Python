...
def Logi(sensOpti = {}, sensDict = {}, sensName = "", contOpti = {}, contDict = {}, contName = "", actuOpti = {}, actuDict = {}, actuName = ""):
	...
	if sensDict != None and sensType != "" and sensType != "linkCont" and sensType != "linkSens":
		#print(sensType)
	...
	# TODO: is this right? list all possibilities and see what options are needed
	#if contType != "" and contType != "linkSens" and contType != "linkCont" and contType != "linkActu" and sensType != "linkCont":
	if contDict != None and contType != "" and contType != "linkSens" and contType != "linkCont" and contType != "linkActu":
	...
	if actuDict != None and actuType != "" and actuType != "linkCont":
	...
	# link
	if sensType == "linkCont":
		#print("sensLink", sensLink)
		# TODO: is this right?
		#Controllers()[sensLink].link(Sensors()[len(Sensors()) - 1])
		Sensors()[sensLink].link(Controllers()[len(Controllers()) - 1])
	contLink = len(Sensors()) - 1
	if contType == "linkSens":
		if contName != "":
			contLink = contName
	# TODO: why was this commented out
	if (contDict != None) and (contType == "linkSens" or sensType == "linkSens" or (sensType != "" and contType != "")):
		#print("sensLink", contLink)
...
def Scri(text, sensName = "", contName = "", use_priority = False):
	import os
	sens = "ALWAYS"
	...
	if sensName == "":
		sensName = text
		sensName = sensName.split(os.sep)
		if len(sensName) > 0:
			sensName = sensName[len(sensName) - 1]
			sensName = sensName.split(".")
		else:
			sensName = ""
		if len(sensName) > 0:
			sensName = sensName[0]
	...
	if contName == "":
		contName = sensName
	Logi(sensName = sensName, sensDict = sensDict, sensOpti = sensOpti, contName = contName, contDict = contDict, contOpti = contOpti)

def Modu(impoList):
	# TODO:
	# make a packaging script that includes common files
	#	start at "prog". include Pyth, prog/game, etc
	retu = []
	import os
	import bpy
	for a in range(len(impoList)):
		lis_ = impoList[a]
		dire = lis_[0]
		lis_.pop(0)
		for b in range(len(lis_)):
			fil_ = lis_[b]
			retu += [dire + fil_]
	for a in range(len(retu)):
		bpy.ops.text.open(filepath = retu[a])
		name = retu[a]
		name = name.split(os.sep)
		if len(name) > 0:
			name = name[len(name) - 1]
			bpy.data.texts[name].use_module = True
...
def PropSet_(propName = "prop", propValu = None):
	if propName in Properties():
		...
		print("warning:", propName, "not found")
...
def ScenObje(charList = [], scenObje = "scen_obje", keyb = True, joys = True, joysInde = 0, joysAxis = 1, joysThre = 8191, lookX___Sens = 1.0, lookY___Sens = 1.0, mousCentX___ = True, mousCentY___ = True):
	...
	Prop(propName = "cont", propType = 'STRING')
...
def LeveInit(name = "", scenNew_ = False, resx = 1360, resy = 768, glsl = True, framExte = True, mousShow = False, debu = False, horizon_color = (0.0, 0.0, 0.0), deleCube = True, deleLigh = True, deleCame = False):
	Blen = BlenLink()
	if scenNew_ == False:
		if deleCube: Blen.DeleCube()
		if deleLigh: Blen.DeleLamp()
		if deleCame: Blen.DeleCame()
		scenNew_ = True
	else:
		Blen.ScenNew_()
	Blen.ScenName(name)
...
def Inpu(keyb = True, joys = True, joysInde = 0, joysAxis = 1, joysThre = 8191, lookX___Sens = 2.0, lookY___Sens = 1.0, mousCentX___ = True, mousCentY___ = True):
	...
	Logi(sensName = sensDict["key"], sensOpti = sensOpti, sensDict = sensDict, contDict = None)
	sensDict["key"] = "A"
	...
	Logi(sensName = sensDict["key"] + "_T", sensOpti = sensOpti, sensDict = sensDict, contDict = None)
	sensDict["key"] = "A"
	#Logi(sensName = sensDict["key"] + "_T", sensOpti = sensOpti, sensDict = sensDict, contDict = "linkSens")
	Logi(sensName = sensDict["key"] + "_T", sensOpti = sensOpti, sensDict = sensDict, contDict = None)
	sensDict["key"] = "S"
	...
	Logi(sensName = 'look', sensOpti = sensOpti, sensDict = sensDict, contDict = None)
...
def Clic():
	...
	actuDict["value"] = "True"
	Logi(sensDict = sensDict, sensOpti = sensOpti, sensName = "clic", contDict = contDict, actuDict = actuDict)
	sensOpti["invert"] = True
	actuDict["value"] = "False"
	Logi(sensDict = sensDict, sensOpti = sensOpti, sensName = "clicInve", contDict = contDict, actuDict = actuDict)
...
def Clir():
	...
	actuDict = LogiActuDict(typ_ = actu)
	sensOpti["use_tap"] = True
	sensDict["mouse_event"] = 'RIGHTCLICK'
	actuDict["property"] = "clir"
	actuDict["value"] = "True"
	Logi(sensDict = sensDict, sensOpti = sensOpti, sensName = "clir", contDict = contDict, actuDict = actuDict)
	sensOpti["invert"] = True
	actuDict["value"] = "False"
	Logi(sensDict = sensDict, sensOpti = sensOpti, sensName = "clirInve", contDict = contDict, actuDict = actuDict)
	Prop(propName = "clir", propType = 'BOOL')
...
# prepare a walkable surface
def PathGeom(scenName = "", use_Prop = True, retu = None):
	import bpy
	Blen = BlenLink()
	Math = MathLink()
	pathObje = bpy.context.object.name
	# TODO: is this being used
	Prop(propName = "use_Prop", propType = 'BOOL', propValu = use_Prop)
	Blen.Appl()
	Blen.Tria()
	...
	# get the polygons belonging to each edge
	edgeBordList = []
	edgeNormList = []
	edgeRefeList = []
	for a in range(len(edgeList)):
		...
		for b in range(len(polyList)):
			if (ver1 in polyList[b]) and (ver2 in polyList[b]):
				edgeBordList[a].append(b)
	# get the edges that belong to each polygon
	polyEdgeList = []
	for a in range(len(polyList)):
		...
		for b in range(len(edgeList)):
			ver1 = edgeList[b][0]
			ver2 = edgeList[b][1]
			if (ver1 in polyList[a]) and (ver2 in polyList[a]):
				polyEdgeList[a].append(b)
	# get the edge normal, and the polygon it points towards
	for a in range(len(polyEdgeList)):
		for b in range(len(polyEdgeList[a])):
			edge = polyEdgeList[a][b]
			ver1 = edgeList[edge][0]
			ver2 = edgeList[edge][1]
			vel1 = vertList[ver1]
			vel2 = vertList[ver2]
			vect = Math.Vect(vel1, vel2)
			# get a normal that lies on the plane now, then adjust next
			norm = normList[a]
			edgeNorm = Math.VectCros3d__(vect, norm)
			cent = centList[a]
			faci = Math.Faci(edgeNorm, vel1, cent)
			if faci == False:
				edgeNorm = Math.VectScal(edgeNorm, -1.0)
			edgeNorm = Math.VectNorm(edgeNorm)
			edgeNormList[edge].append(edgeNorm)
			edgeRefeList[edge].append(a)
	# adjust edge normals to separate 3d space
	for a in range(len(edgeNormList)):
		pol1 = edgeRefeList[a][0]
		# first edge normal
		nor1 = edgeNormList[a][0]
		if len(edgeNormList[a]) == 2:
			nor2 = edgeNormList[a][1]
			# cross
			edge = edgeList[a]
			ver1 = vertList[edge[0]]
			ver2 = vertList[edge[1]]
			axis = Math.Vect(ver1, ver2)
			axis = Math.VectNorm(axis)
			# rotate
			angl = Math.VectAngl(nor1, nor2)
			if Math.VectDot_(nor1, nor2) < 0.0:
				angl = 360.0 - angl
			cro1 = Math.VectCros3d__(axis, nor1)
			cro2 = Math.VectCros3d__(axis, nor2)
			cros = Math.VectCros3d__(Math.VectNorm(cro1), Math.VectNorm(cro2))
			if Math.VectDot_(cros, axis) >= 0.0:
				angl *= -1.0
			angl /= 2.0
			# TODO: use signed angle. here, planAngl, etc
			# TODO: if this works, only need one reference polygon
			no_1 = normList[pol1]
			pol2 = edgeRefeList[a][1]
			no_2 = normList[pol2]
			cros = Math.Quat(nor1, angl, axis)
			new_Norm = Math.VectCros3d__(Math.VectNorm(cros), axis)
			new_Norm = Math.VectNorm(new_Norm)
			if Math.VectDot_(nor1, new_Norm) < 0.0:
				new_Norm = Math.VectScal(new_Norm, -1.0)
			nor1 = new_Norm
		edgeNormList[a][0] = nor1
		"""
		# draw edge normal
		#import math
		#if math.fabs(nor1[2]) > 0.0001:
		Blen.VectDraw(nor1)
		edge = edgeList[a]
		ver1 = vertList[edge[0]]
		ver2 = vertList[edge[1]]
		vect = Math.Vect(ver1, ver2)
		vect = Math.VectScal(vect, 0.5)
		vect = Math.VectAdd_(vect, ver1)
		Blen.Loca(vect)
		"""
	# use the first edge normal to define both polygons
	for a in range(len(edgeNormList)):
		edgeNormList[a] = edgeNormList[a][0]
		edgeRefeList[a] = edgeRefeList[a][0]
	if use_Prop:
		if retu == None:
			retu = []
		# add geometry to empty objects
		emptList = [pathObje + ".vertList", pathObje + ".edgeList", pathObje + ".polyList", pathObje + ".normList", pathObje + ".polyEdgeList", pathObje + ".edgeNormList", pathObje + ".edgeRefeList", pathObje + ".edgeBordList"]
		Blen.Empt()
		Blen.Name(emptList[0])
		Prop(propName = "leng", propType = 'INT', propValu = len(vertList))
		for a in range(len(vertList)):
			Prop(propName = "vert." + str(a) + ".x", propType = 'FLOAT', propValu = vertList[a][0])
			Prop(propName = "vert." + str(a) + ".y", propType = 'FLOAT', propValu = vertList[a][1])
			Prop(propName = "vert." + str(a) + ".z", propType = 'FLOAT', propValu = vertList[a][2])
		Blen.Empt()
		Blen.Name(emptList[1])
		Prop(propName = "leng", propType = 'INT', propValu = len(edgeList))
		for a in range(len(edgeList)):
			Prop(propName = "edge." + str(a) + ".0", propType = 'INT', propValu = edgeList[a][0])
			Prop(propName = "edge." + str(a) + ".1", propType = 'INT', propValu = edgeList[a][1])
		Blen.Empt()
		Blen.Name(emptList[2])
		Prop(propName = "leng", propType = 'INT', propValu = len(polyList))
		for a in range(len(polyList)):
			Prop(propName = "poly." + str(a) + ".0", propType = 'INT', propValu = polyList[a][0])
			Prop(propName = "poly." + str(a) + ".1", propType = 'INT', propValu = polyList[a][1])
			Prop(propName = "poly." + str(a) + ".2", propType = 'INT', propValu = polyList[a][2])
		Blen.Empt()
		Blen.Name(emptList[3])
		Prop(propName = "leng", propType = 'INT', propValu = len(normList))
		for a in range(len(normList)):
			Prop(propName = "norm." + str(a) + ".x", propType = 'FLOAT', propValu = normList[a][0])
			Prop(propName = "norm." + str(a) + ".y", propType = 'FLOAT', propValu = normList[a][1])
			Prop(propName = "norm." + str(a) + ".z", propType = 'FLOAT', propValu = normList[a][2])
		Blen.Empt()
		Blen.Name(emptList[4])
		Prop(propName = "leng", propType = 'INT', propValu = len(polyEdgeList))
		for a in range(len(polyEdgeList)):
			Prop(propName = "polyEdge." + str(a) + ".0", propType = 'INT', propValu = polyEdgeList[a][0])
			Prop(propName = "polyEdge." + str(a) + ".1", propType = 'INT', propValu = polyEdgeList[a][1])
			Prop(propName = "polyEdge." + str(a) + ".2", propType = 'INT', propValu = polyEdgeList[a][2])
		Blen.Empt()
		Blen.Name(emptList[5])
		Prop(propName = "leng", propType = 'INT', propValu = len(edgeNormList))
		for a in range(len(edgeNormList)):
			Prop(propName = "edgeNorm." + str(a) + ".x", propType = 'FLOAT', propValu = edgeNormList[a][0])
			Prop(propName = "edgeNorm." + str(a) + ".y", propType = 'FLOAT', propValu = edgeNormList[a][1])
			Prop(propName = "edgeNorm." + str(a) + ".z", propType = 'FLOAT', propValu = edgeNormList[a][2])
		Blen.Empt()
		Blen.Empt()
		Blen.Name(emptList[6])
		Prop(propName = "leng", propType = 'INT', propValu = len(edgeRefeList))
		for a in range(len(edgeRefeList)):
			Prop(propName = "edgeRefe." + str(a), propType = 'INT', propValu = edgeRefeList[a])
		Blen.Empt()
		Blen.Name(emptList[7])
		Prop(propName = "leng", propType = 'INT', propValu = len(edgeBordList))
		for a in range(len(edgeBordList)):
			Prop(propName = "edgeBord." + str(a) + ".0", propType = 'INT', propValu = edgeBordList[a][0])
			if len(edgeBordList[a]) == 2:
				Prop(propName = "edgeBord." + str(a) + ".1", propType = 'INT', propValu = edgeBordList[a][1])
			else:
				Prop(propName = "edgeBord." + str(a) + ".1", propType = 'INT', propValu = -1)
		for a in range(len(emptList)):
			retu.append(scenName + "." + emptList[a])
	if use_Prop == False:
		"""
		# add geometry to a script
		retu = "\n"
		retu += "def PathGeom():\n"
		retu += "\tvertList = ["
		for a in range(len(vertList)):
			retu += "(" + str(vertList[a][0]) + ", " + str(vertList[a][1]) + ", " + str(vertList[a][2]) + ")"
			if a < len(vertList) - 1:
				retu += ", "
		retu += "]\n"
		retu += "\tedgeList = ["
		for a in range(len(edgeList)):
			retu += "[" + str(edgeList[a][0]) + ", " + str(edgeList[a][1]) + "]"
			if a < len(edgeList) - 1:
				retu += ", "
		retu += "]\n"
		retu += "\tpolyList = ["
		for a in range(len(polyList)):
			retu += "[" + str(polyList[a][0]) + ", " + str(polyList[a][1]) + ", " + str(polyList[a][2]) + "]"
			if a < len(polyList) - 1:
				retu += ", "
		retu += "]\n"
		retu += "\tnormList = ["
		for a in range(len(normList)):
			retu += "(" + str(normList[a][0]) + ", " + str(normList[a][1]) + ", " + str(normList[a][2]) + ")"
			if a < len(normList) - 1:
				retu += ", "
		retu += "]\n"
		retu += "\tpolyEdgeList = ["
		for a in range(len(polyEdgeList)):
			retu += "[" + str(polyEdgeList[a][0]) + ", " + str(polyEdgeList[a][1]) + ", " + str(polyEdgeList[a][2]) + "]"
			if a < len(polyEdgeList) - 1:
				retu += ", "
		retu += "]\n"
		retu += "\tedgeNormList = ["
		for a in range(len(edgeNormList)):
			retu += "(" + str(edgeNormList[a][0]) + ", " + str(edgeNormList[a][1]) + ", " + str(edgeNormList[a][2]) + ")"
			if a < len(edgeNormList) - 1:
				retu += ", "
		retu += "]\n"
		retu += "\tedgeRefeList = ["
		for a in range(len(edgeRefeList)):
			retu += str(edgeRefeList[a])
			if a < len(edgeRefeList) - 1:
				retu += ", "
		retu += "]\n"
		retu += "\tedgeBordList = ["
		for a in range(len(edgeBordList)):
			bord = [edgeBordList[a][0]]
			if len(edgeBordList[a]) == 2:
				bord.append(edgeBordList[a][1])
			else:
				bord.append(-1)
			retu += "[" + str(bord[0]) + ", " + str(bord[1]) + "]"
			if a < len(edgeBordList) - 1:
				retu += ", "
		retu += "]\n"
		retu += "\treturn vertList, edgeList, polyList, normList, polyEdgeList, edgeNormList, edgeRefeList, edgeBordList\n"
		"""
		# add geometry to a script
		# TODO: each function to its own file
		"""
		retu = "\n"
		retu += "def VertList(inde):\n"
		retu += "\tretu = (0.0, 0.0, 0.0)\n"
		for a in range(len(vertList)):
			retu += "\tif inde == " + str(a) + ":\n"
			retu += "\t\tretu = (" + str(vertList[a][0]) + ", " + str(vertList[a][1]) + ", " + str(vertList[a][2]) + ")\n"
		retu += "\treturn retu\n"
		retu += "def EdgeList(inde):\n"
		retu += "\tretu = (-1, -1)\n"
		for a in range(len(edgeList)):
			retu += "\tif inde == " + str(a) + ":\n"
			retu += "\t\tretu = (" + str(edgeList[a][0]) + ", " + str(edgeList[a][1]) + ")\n"
		retu += "\treturn retu\n"
		retu += "def PolyList(inde):\n"
		retu += "\tretu = (-1, -1, -1)\n"
		for a in range(len(polyList)):
			retu += "\tif inde == " + str(a) + ":\n"
			retu += "\t\tretu = (" + str(polyList[a][0]) + ", " + str(polyList[a][1]) + ", " + str(polyList[a][2]) + ")\n"
		retu += "\treturn retu\n"
		retu += "def NormList(inde):\n"
		retu += "\tretu = (0.0, 0.0, 0.0)\n"
		for a in range(len(normList)):
			retu += "\tif inde == " + str(a) + ":\n"
			retu += "\t\tretu = (" + str(normList[a][0]) + ", " + str(normList[a][1]) + ", " + str(normList[a][2]) + ")\n"
		retu += "\treturn retu\n"
		retu += "def PolyEdgeList(inde):\n"
		retu += "\tretu = (-1, -1, -1)\n"
		for a in range(len(polyEdgeList)):
			retu += "\tif inde == " + str(a) + ":\n"
			retu += "\t\tretu = (" + str(polyEdgeList[a][0]) + ", " + str(polyEdgeList[a][1]) + ", " + str(polyEdgeList[a][2]) + ")\n"
		retu += "\treturn retu\n"
		retu += "def EdgeNormList(inde):\n"
		retu += "\tretu = (0.0, 0.0, 0.0)\n"
		for a in range(len(edgeNormList)):
			retu += "\tif inde == " + str(a) + ":\n"
			retu += "\t\tretu = (" + str(edgeNormList[a][0]) + ", " + str(edgeNormList[a][1]) + ", " + str(edgeNormList[a][2]) + ")\n"
		retu += "\treturn retu\n"
		retu += "def EdgeRefeList(inde):\n"
		retu += "\tretu = -1\n"
		for a in range(len(edgeRefeList)):
			retu += "\tif inde == " + str(a) + ":\n"
			retu += "\t\tretu = " + str(edgeRefeList[a]) + "\n"
		retu += "\treturn retu\n"
		retu += "def EdgeBordList(inde):\n"
		retu += "\tretu = (-1, -1)\n"
		for a in range(len(edgeBordList)):
			bord = [edgeBordList[a][0]]
			if len(edgeBordList[a]) == 2:
				...
				bord.append(-1)
			retu += "\tif inde == " + str(a) + ":\n"
			retu += "\t\tretu = (" + str(bord[0]) + ", " + str(bord[1]) + ")\n"
		retu += "\treturn retu\n"
		"""
		...
		retu += ["\tretu = (0.0, 0.0, 0.0)"]
		for a in range(len(vertList)):
			retu += ["\tif inde == " + str(a) + ": retu = (" + str(vertList[a][0]) + ", " + str(vertList[a][1]) + ", " + str(vertList[a][2]) + ")"]
			#retu += ["\t\t
		retu += ["\treturn retu"]
		retu += [""]
		retu += ["def EdgeList(inde):"]
		retu += ["\tretu = (-1, -1)"]
		for a in range(len(edgeList)):
			retu += ["\tif inde == " + str(a) + ": retu = (" + str(edgeList[a][0]) + ", " + str(edgeList[a][1]) + ")"]
			#retu += ["\t\t
		retu += ["\treturn retu"]
		retu += [""]
		retu += ["def PolyList(inde):"]
		retu += ["\tretu = (-1, -1, -1)"]
		for a in range(len(polyList)):
			retu += ["\tif inde == " + str(a) + ": retu = (" + str(polyList[a][0]) + ", " + str(polyList[a][1]) + ", " + str(polyList[a][2]) + ")"]
			#retu += ["\t\t
		retu += ["\treturn retu"]
		retu += [""]
		retu += ["def NormList(inde):"]
		retu += ["\tretu = (0.0, 0.0, 0.0)"]
		for a in range(len(normList)):
			retu += ["\tif inde == " + str(a) + ": retu = (" + str(normList[a][0]) + ", " + str(normList[a][1]) + ", " + str(normList[a][2]) + ")"]
			#retu += ["\t\t
		retu += ["\treturn retu"]
		retu += [""]
		retu += ["def PolyEdgeList(inde):"]
		retu += ["\tretu = (-1, -1, -1)"]
		for a in range(len(polyEdgeList)):
			retu += ["\tif inde == " + str(a) + ": retu = (" + str(polyEdgeList[a][0]) + ", " + str(polyEdgeList[a][1]) + ", " + str(polyEdgeList[a][2]) + ")"]
			#retu += ["\t\t
		retu += ["\treturn retu"]
		retu += [""]
		retu += ["def EdgeNormList(inde):"]
		retu += ["\tretu = (0.0, 0.0, 0.0)"]
		for a in range(len(edgeNormList)):
			retu += ["\tif inde == " + str(a) + ": retu = (" + str(edgeNormList[a][0]) + ", " + str(edgeNormList[a][1]) + ", " + str(edgeNormList[a][2]) + ")"]
			#retu += ["\t\t
		retu += ["\treturn retu"]
		retu += [""]
		retu += ["def EdgeRefeList(inde):"]
		retu += ["\tretu = -1"]
		for a in range(len(edgeRefeList)):
			retu += ["\tif inde == " + str(a) + ": retu = " + str(edgeRefeList[a])]
			#retu += ["\t\t
		retu += ["\treturn retu"]
		retu += [""]
		retu += ["def EdgeBordList(inde):"]
		...
		for a in range(len(edgeBordList)):
			...
			if len(edgeBordList[a]) == 2:
				...
				bord.append(-1)
			retu += ["\tif inde == " + str(a) + ": retu = (" + str(bord[0]) + ", " + str(bord[1]) + ")"]
			#retu += ["\t\t
		retu += ["\treturn retu"]
		retu += [""]
...
# create a mesh based on walls
# TODO:
# only works for axis-aligned walls
# creates duplicate vertices
# not very efficient
# creates an extra face in the center, and upside-down normals
def WallPath(pathObje = "", wallCoun = 0, xmin = -100.0, xmax = 100.0, ymin = -100.0, ymax = 100.0, pref = "wall"):
	import bpy
	Blen = BlenLink()
	Blen.Sele(pathObje)
	# add bounding vertices
	vertList = [(xmax, ymax, 0.0), (xmin, ymax, 0.0), (xmin, ymin, 0.0), (xmax, ymin, 0.0)]
	# get max / min x / y vertex of each wall and add it to vertList
	for a in range(wallCoun):
		Blen.Sele(pref + "." + Blen.Pad_(a))
		loca = Blen.LocaRead()
		ver_List = Blen.VertList()
		ve_xList = []
		ve_yList = []
		for b in range(len(ver_List)):
			ve_xList.append(ver_List[b][0])
			ve_yList.append(ver_List[b][1])
		ve_xList = sorted(ve_xList)
		ve_yList = sorted(ve_yList)
		xmi_ = ve_xList[0] + loca[0]
		xma_ = ve_xList[len(ve_xList) - 1] + loca[0]
		ymi_ = ve_yList[0] + loca[1]
		yma_ = ve_yList[len(ve_yList) - 1] + loca[1]
		vertList.append((xmi_, ymi_, 0.0))
		vertList.append((xmi_, yma_, 0.0))
		vertList.append((xma_, ymi_, 0.0))
		vertList.append((xma_, yma_, 0.0))
		if len(vertList) > 100000:
			print("vertList grew beyond limit. exiting")
			return -1
	# add intersections
	leng = len(vertList)
	for a in range(leng):
		for b in range(leng):
			if a != b:
				if ((vertList[a][0], vertList[b][1], 0.0) in vertList) == False:
					vertList.append((vertList[a][0], vertList[b][1], 0.0))
				if ((vertList[b][0], vertList[a][1], 0.0) in vertList) == False:
					vertList.append((vertList[b][0], vertList[a][1], 0.0))
	# upload so that blender can remove doubles
	Blen.Uplo([vertList, [], []])
	Blen.VertDoub()
	# re-download
	vertList = Blen.VertList()
	Blen.Dele()
	# sort vertList by two dimensions
	...
	while a < len(vertList):
		ve_yList = []
		star = a
		while a < len(vertList) and vertList[a][0] == x:
			ve_yList.append(vertList[a][1])
			a += 1
		for b in range(len(ve_yList)): vertList[star + b] = (vertList[star + b][0], ve_yList[b], 0.0)
		if a < len(vertList): x = vertList[a][0]
	# add edges
	edgeList = []
	x = vertList[0][0]
	a = 0
	...
	while a < len(vertList):
		...
		star = a
		while a < len(vertList) and vertList[a][0] == x:
			if leng != -1: edgeList.append((a, a + leng))
...
# TODO: use transforms
def WallList(wallPath = "wall.path", wallCoun = 0, pref = "wall"):
	import bpy
	Blen = BlenLink()
	Math = MathLink()
	# determine the polygons that lie in front of a wall
	Blen.Sele(wallPath)
	pathLoca = Blen.LocaRead()
	polyList = Blen.PolyList()
	centList = Blen.CentList()
	# list of walls belonging to polys
	polyWall = []
	for a in range(len(polyList)):
		polyWall.append([])
	vertList = Blen.VertList()
	for a in range(wallCoun):
		...
		ymi_ = ve_yList[0] + loca[1]
		yma_ = ve_yList[len(ve_yList) - 1] + loca[1]
		norm = bpy.context.object.data.polygons[0].normal
		for b in range(len(polyList)):
			# if the wall is facing the poly
			faci = False
			# TODO: trying polygon centers instead of each vertex
			cent = centList[b]
			cent = Math.VectAdd_(cent, pathLoca)
			faci = Math.Faci(norm, loca, cent)
			# TODO: if the poly is within the x / y bounds of the wall
			tole = 0.1
			boun = False
			radi = 0.0
			if Math.VectMagn(Math.Vect(loca, (xmi_, ymi_, 0.0))) > radi: radi = Math.VectMagn(Math.Vect(loca, (xmi_, ymi_, 0.0)))
			if Math.VectMagn(Math.Vect(loca, (xmi_, yma_, 0.0))) > radi: radi = Math.VectMagn(Math.Vect(loca, (xmi_, yma_, 0.0)))
			if Math.VectMagn(Math.Vect(loca, (xma_, ymi_, 0.0))) > radi: radi = Math.VectMagn(Math.Vect(loca, (xma_, ymi_, 0.0)))
			if Math.VectMagn(Math.Vect(loca, (xma_, yma_, 0.0))) > radi: radi = Math.VectMagn(Math.Vect(loca, (xma_, yma_, 0.0)))
			# if the poly is within a certain range
			rang = False
			# TODO: should be camera max distance
			rangDist = 8.0
			# ver0
			ver0 = polyList[b][0]
			ver0 = vertList[ver0]
			ver0 = Math.VectAdd_(ver0, pathLoca)
			# faci
			#if Math.Faci(norm, loca, ver0): faci = True
			# rang
			vect = Math.Vect(loca, ver0)
			vect = Math.VectProj(vect, norm)
			if Math.VectMagn(vect) < rangDist: rang = True
			# boun
			vect = Math.VectAdd_(vect, loca)
			vect = Math.Vect(vect, ver0)
			if Math.VectMagn(vect) < radi + tole: boun = True
			# ver1
			ver1 = polyList[b][1]
			ver1 = vertList[ver1]
			ver1 = Math.VectAdd_(ver1, pathLoca)
			# faci
			#if Math.Faci(norm, loca, ver1): faci = True
			# rang
			vect = Math.Vect(loca, ver1)
			vect = Math.VectProj(vect, norm)
			if Math.VectMagn(vect) < rangDist: rang = True
			# boun
			vect = Math.VectAdd_(vect, loca)
			vect = Math.Vect(vect, ver1)
			if Math.VectMagn(vect) < radi + tole: boun = True
			# ver2
			ver2 = polyList[b][2]
			ver2 = vertList[ver2]
			ver2 = Math.VectAdd_(ver2, pathLoca)
			# faci
			#if Math.Faci(norm, loca, ver2): faci = True
			# rang
			vect = Math.Vect(loca, ver2)
			vect = Math.VectProj(vect, norm)
			if Math.VectMagn(vect) < rangDist: rang = True
			# boun
			vect = Math.VectAdd_(vect, loca)
			vect = Math.Vect(vect, ver2)
			if Math.VectMagn(vect) < radi + tole: boun = True
			if faci and boun and rang: polyWall[b].append(a)
	return polyWall

# TODO: only creates vertices. need to create border
def WallOutl(wallCoun = 0, wallPath = "wall.path", pref = "wall", name = "wall.outl"):
	Blen = BlenLink()
	Math = MathLink()
	# make outline from walls
	Blen.Sele(wallPath)
	vertList = []
	for a in range(wallCoun):
		Blen.Sele(pref + "." + Blen.Pad_(a))
		...
		ver_List = Blen.VertList()
		for b in range(len(ver_List)):
			vertList.append((ver_List[b][0] + loca[0], ver_List[b][1] + loca[1], 0.0))
	# TODO: make this block a python function. extend for as many dimensions that are in the list
	vertList = sorted(vertList)
	x = vertList[0][0]
	a = 0
	while a < len(vertList):
	...
	a = len(vertList) - 1
	while a > 0:
		if Math.VectMagn(Math.Vect(vertList[a], vertList[a - 1])) < tole:
			vertList.pop(a)
		a -= 1
	# TODO: create border
	# pass a known point
	# different approaches:
	# 1: create a circle and "shrinkwrap" from the outside. might close off coves.
	# 2: extrude a known point from the inside, into coves until a straight line can be drawn between all nodes and border. test extensions by checking if they connect to border vertices from both sides.
	# maybe combine both. use 1 until it fails. test if a line can be drawn to known point without intersecting. use 2 to resolve
	Blen.Uplo([vertList, [], []])
	Blen.Name(name)

def Path(grouCast = True, posiAdju = True, posiAdjuSlid = True, coll = False, orie = False, orieCons = False):
	Prop(propName = "pathPoly", propType = 'INT')
	#Prop(propName = "pathName", propType = 'STRING', propValu = pathName)
	Prop(propName = "grouCast", propType = 'BOOL', propValu = grouCast)
	Prop(propName = "posiAdju", propType = 'BOOL', propValu = posiAdju)
	Prop(propName = "posiAdjuSlid", propType = 'BOOL', propValu = posiAdjuSlid)
	Prop(propName = "orie", propType = 'BOOL', propValu = orie)
	Prop(propName = "orieCons", propType = 'BOOL', propValu = orieCons)
...
def MMXY():
	...
	for a in range(len(bpy.context.object.data.polygons)):
		...
		minyInd_ = -1
		maxyInd_ = -1
		for b in range(len(bpy.context.object.data.polygons[a].vertices)):
			vert = bpy.context.object.data.polygons[a].vertices[b]
			x = bpy.context.object.data.vertices[vert].co.x
			y = bpy.context.object.data.vertices[vert].co.y
			z = bpy.context.object.data.vertices[vert].co.z
			co = Math.VectRota3d__((x, y, z), rota)
			x = co[0] * scal[0]
			y = co[1] * scal[1]
			x += loca[0]
			y += loca[1]
			if minxInd_ == -1 or x < minx:
				minxInd_ = b
				minx = x
			if maxxInd_ == -1 or x > maxx:
				maxxInd_ = b
				maxx = x
			if minyInd_ == -1 or y < miny:
				minyInd_ = b
				miny = y
			if maxyInd_ == -1 or y > maxy:
				maxyInd_ = b
				maxy = y
		minxList.append([minx, a])
		maxxList.append([maxx, a])
		minyList.append([miny, a])
		maxyList.append([maxy, a])
	minxList, minxInde, maxxList, maxxInde, minyList, minyInde, maxyList, maxyInde = SortMMXY(minxList, minxInde, maxxList, maxxInde, minyList, minyInde, maxyList, maxyInde)
	return minxList, minxInde, maxxList, maxxInde, minyList, minyInde, maxyList, maxyInde

def SortMMXY(minxList, minxInde, maxxList, maxxInde, minyList, minyInde, maxyList, maxyInde):
	minxList = sorted(minxList)
	maxxList = sorted(maxxList)
	minyList = sorted(minyList)
	maxyList = sorted(maxyList)
	for a in range(len(minxList)):
		minxInde[minxList[a][1]] = a
	for a in range(len(maxxList)):
		maxxInde[maxxList[a][1]] = a
	for a in range(len(minyList)):
		minyInde[minyList[a][1]] = a
	for a in range(len(maxyList)):
		maxyInde[maxyList[a][1]] = a
	return minxList, minxInde, maxxList, maxxInde, minyList, minyInde, maxyList, maxyInde

def PolyMark(obje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark):
	import bpy
	Blen = BlenLink()
	# get a bounding box for all polygons
	Blen.Sele(obje)
	polyList = Blen.PolyList()
	# get a bounding box for a marker
	Blen.Sele(mark)
	minxLis2, minxInd2, maxxLis2, maxxInd2, minyLis2, minyInd2, maxyLis2, maxyInd2 = MMXY()
	minxLis2 = Mini(minxLis2)
	maxxLis2 = Maxi(maxxLis2)
	minyLis2 = Mini(minyLis2)
	maxyLis2 = Maxi(maxyLis2)
	# check overlap
	ovex = Overlap_(minxLis1, minxLis2, maxxLis1, maxxLis2, minxInd1, maxxInd1, polyList)
	ovey = Overlap_(minyLis1, minyLis2, maxyLis1, maxyLis2, minyInd1, maxyInd1, polyList)
	ovex = sorted(ovex)
	ovey = sorted(ovey)
	polyList = []
	for a in range(len(ovex)):
		for b in range(len(ovey)):
			if ovex[a] == ovey[b]:
				polyList.append(ovex[a])
				break
			if ovex[a] < ovey[b]:
				break
	return polyList
...
def Maxi(maxi):
	retu = 0.0
	inde = -1
	for a in range(len(maxi)):
		if inde == -1 or maxi[a][0] > maxi:
			retu = maxi[a][0]
			inde = a
	return retu, inde

# TODO:
# sorting doesnt help here, all polys are being checked
# start at minxLis2, look left, then right, etc
def Overlap_(miniLis1, miniLis2, maxiLis1, maxiLis2, miniInde, maxiInde, polyList):
	over = []
	for a in range(len(polyList)):
		orde = []
		orde.append([miniLis2[0], 0])
		orde.append([maxiLis2[0], 0])
		orde.append([miniLis1[miniInde[a]][0], 1])
		orde.append([maxiLis1[maxiInde[a]][0], 1])
		orde = sorted(orde)
		orde = [orde[0][1], orde[1][1], orde[2][1], orde[3][1]]
		if orde == [0, 1, 0, 1] or orde == [1, 0, 1, 0] or orde == [1, 0, 0, 1] or orde == [0, 1, 1, 0]:
			over.append(a)
	return over

def PolySet_(variObje, pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark, propName):
	Blen = BlenLink()
	Blen.Sele(variObje)
	polyList = PolyMark(pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark)
	Blen.Sele(variObje)
	Prop(propName = propName, propType = 'INT', propValu = len(polyList))
	for a in range(len(polyList)):
		Prop(propName = propName + "." + str(a), propType = 'INT', propValu = polyList[a])
	Blen.Sele(mark)
	Blen.Dele()

# star poly
def PathPolySet_(pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark):
	Blen = BlenLink()
	polyList = PolyMark(pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark)
	Blen.Sele(mark)
	Blen.Dele()
...
# unwalkable
def PathUnwa(pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, markList, scenName, hideList):
	Blen = BlenLink()
	Blen.Empt()
	variObje = "unwa"
	Blen.Name(variObje)
	...
	for mark in markList:
		polyList = PolyMark(pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark)
		Blen.Sele(variObje)
		Prop(propName = "unwaCoun." + str(unwaSet_), propType = 'INT', propValu = len(polyList))
		for b in range(len(polyList)):
			Prop(propName = "unwa." + str(unwaSet_) + "." + str(b), propType = 'INT', propValu = polyList[b])
		Prop(propName = "unwa." + str(unwaSet_), propType = 'BOOL', propValu = True)
		unwaSet_ += 1
		Blen.Sele(mark)
		Blen.Dele()
	Blen.Sele(variObje)
	Prop(propName = "set_Coun", propType = 'INT', propValu = len(markList))
	hideList.append(scenName + "." + variObje)
	return hideList

def PathTrig(variObje, pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark, vari, scenName, hideList):
	Blen = BlenLink()
	Blen.Empt()
	Blen.Name(variObje)
	PolySet_(variObje, pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark, vari)
	hideList.append(scenName + "." + variObje)
	return hideList
...
# BOUNDS FUNCTIONS
# functions for (loosely) tracking and updating the start and end of the bounding boxes of a list of objects
# reasons might be checking proximity, pre-collisions, pre-raycasting, et cetera.

# TODO: rewrite description and include an example
# list of object bounds to be tracked
# second entry is False for dynamic, True for static
# static objects will use a bounding box according to their vertices
# for dynamic objects, a bounding box will be created using the vertex with the largest magnitude
# this is a sort of "worst-case scenario bounding box", or a bounding box that encloses the object regardless of it's rotation
# TODO: currently assumes track object is only in one area at any given time
def BounWrit(objeList = [], bounAreaName = "boun_area", trac = None):
	retu = {}
	# TODO:
	# option to sort areas by connected
	# TODO: rays
	# need to track ray position(s) in boux bouy bouz
	#	what if ray position was always 0?
	# would it be possible to ignore object rotation entirely? rotate rays instead. would have to track magnitude among all rays as opposed to distance along a world axis for a single ray
	import bpy
	import math
	Blen = BlenLink()
	Math = MathLink()
	...
	bouz = []
	areaCoun = 0
	leng = len(bpy.context.scene.objects)
	a = 0
	while a < leng:
		obje = bpy.context.scene.objects[a]
		name = obje.name
		name = name.split(".")
		if len(name) > 0 and name[0] == bounAreaName:
			area = int(name[1])
			# get mins and maxs
			Blen.Sele(obje.name)
			Blen.Appl()
			minx = Blen.Xyz_Most(axis = 0, inde = False, reverse = False)
			maxx = Blen.Xyz_Most(axis = 0, inde = False, reverse = True)
			miny = Blen.Xyz_Most(axis = 1, inde = False, reverse = False)
			maxy = Blen.Xyz_Most(axis = 1, inde = False, reverse = True)
			minz = Blen.Xyz_Most(axis = 2, inde = False, reverse = False)
			maxz = Blen.Xyz_Most(axis = 2, inde = False, reverse = True)
			retu.update({"bounArea." + str(area) + ".minx" : minx})
			retu.update({"bounArea." + str(area) + ".minx" : maxx})
			retu.update({"bounArea." + str(area) + ".minx" : miny})
			retu.update({"bounArea." + str(area) + ".minx" : maxy})
			retu.update({"bounArea." + str(area) + ".minx" : minz})
			retu.update({"bounArea." + str(area) + ".minx" : maxz})
			boux.append([minx, area, 0])
			boux.append([maxx, area, 1])
			bouy.append([miny, area, 0])
			bouy.append([maxy, area, 1])
			bouz.append([minz, area, 0])
			bouz.append([maxz, area, 1])
			areaCoun += 1
			Blen.Dele()
			a -= 1
			leng = len(bpy.context.scene.objects)
	...
	bouz = sorted(bouz)
	bounAreaAxis = []
	for a in range(areaCoun):
		retu.update({"bounAreaAxis." + str(a) + ".x" : True})
		retu.update({"bounAreaAxis." + str(a) + ".y" : True})
		retu.update({"bounAreaAxis." + str(a) + ".z" : True})
	# TODO: currently assumes track object is only in one area at any given time. implement area nesting
	areaCurr = 0
	if trac != None:
		Blen.Sele(trac)
		loca = Blen.LocaRead()
		# TODO: should 1 be center and 2 be max?
		retu.update({"bounTrac" : trac})
		retu.update({"bounTrac.x" : loca[0]})
		retu.update({"bounTrac.y" : loca[1]})
		retu.update({"bounTrac.z" : loca[2]})
		for a in range(len(boux)):
			# if the bounds is a minimum and the object is less than the bounds, or vice versa, the object is outside the area for this axis
			# TODO: are the equalities right
			if (boux[a][2] == 0 and loca[0] < boux[a][0]) or (boux[a][2] == 1 and loca[0] >= boux[a][0]):
				area = boux[a][1]
				retu["bounAreaAxis." + str(area) + ".x"] = False
			if (bouy[a][2] == 0 and loca[1] < bouy[a][0]) or (bouy[a][2] == 1 and loca[1] >= bouy[a][0]):
				area = bouy[a][1]
				retu["bounAreaAxis." + str(area) + ".y"] = False
			if (bouz[a][2] == 0 and loca[2] < bouz[a][0]) or (bouz[a][2] == 1 and loca[2] >= bouz[a][0]):
				area = bouz[a][1]
				retu["bounAreaAxis." + str(area) + ".z"] = False
		for a in range(len(bounAreaAxis)):
			if (False in bounAreaAxis[a]) == False:
				areaCurr = a
				break
		for a in range(len(boux)):
			if (boux[a][2] == 0 and boux[a][1] == areaCurr): retu.update({"bounTrac.indx" : a})
			if (bouy[a][2] == 0 and bouy[a][1] == areaCurr): retu.update({"bounTrac.indy" : a})
			if (bouz[a][2] == 0 and bouz[a][1] == areaCurr): retu.update({"bounTrac.indz" : a})
		retu.update({"bounTracUse_" : True})
	else:
		retu.update({"bounTracUse_" : False})
	retu.update({"bounArea" : areaCurr})
	retu.update({"bounAreaCoun" : areaCoun + 1})
	retu.update({"bounAreaLeng" : len(boux)})
	# TODO: make flags consistent with other bounds arrays
	for a in range(len(boux)):
		retu.update({"bouxArea." + str(a) + ".posi" : boux[a][0]})
		retu.update({"bouxArea." + str(a) + ".area" : boux[a][1]})
		retu.update({"bouxArea." + str(a) + ".maxi" : boux[a][2]})
		retu.update({"bouyArea." + str(a) + ".posi" : bouy[a][0]})
		retu.update({"bouyArea." + str(a) + ".area" : bouy[a][1]})
		retu.update({"bouyArea." + str(a) + ".maxi" : bouy[a][2]})
		retu.update({"bouzArea." + str(a) + ".posi" : bouz[a][0]})
		retu.update({"bouzArea." + str(a) + ".area" : bouz[a][1]})
		retu.update({"bouzArea." + str(a) + ".maxi" : bouz[a][2]})

	# count the number of objects per area
	for a in range(retu["bounAreaCoun"]):
		leng = 0
		#while True:
		#	obje = "boun." + str(a) + "." + str(leng)
		#	if (obje in retu) == False: break
		#	leng += 1
		#print("leng", leng)
		retu.update({"boun." + str(a) + ".leng" : 0})
	...
	for a in range(len(objeList)):
		...
		obje = Blen.Sele(name)
		if obje != None:
			# is the object static (True) or dynamic (False)
			stat = objeList[a][1]
			# is the object to be tracked as one point (False) or two (True)
			# TODO: add to main
			rang = objeList[a][2]
			loca = Blen.LocaRead()
			magn = minx = maxx = miny = maxy = minz = maxz = 0.0
			#### triangulate object
			# TODO: add to raycast
			#Blen.Tria()
			if stat:
				# TODO: add to raycast. need to add 3d grid to objects and make a list of vertices in each grid
				minx = Blen.Xyz_Most(axis = 0, reverse = False, inde = False)
				maxx = Blen.Xyz_Most(axis = 0, reverse = True, inde = False)
				miny = Blen.Xyz_Most(axis = 1, reverse = False, inde = False)
				maxy = Blen.Xyz_Most(axis = 1, reverse = True, inde = False)
				minz = Blen.Xyz_Most(axis = 2, reverse = False, inde = False)
				maxz = Blen.Xyz_Most(axis = 2, reverse = True, inde = False)
			else:
				# worst-case bounds
				# get the MAGNITUDE to all vertices
				# find the largest magnitude
				# worst-case scenario bounds is that magnitude in any dimension from the origin
				magnList = []
				for b in range(len(obje.data.vertices)):
					magnList.append(Math.VectMagn(obje.data.vertices[b].co))
				magnList = sorted(magnList)
				magn = magnList[len(magnList) - 1]
			founArea = False
			area = 0
			if trac != None:
				for b in range(retu["bounAreaCoun"]):
					if loca[0] >= retu["bounArea." + str(b) + ".minx"] and loca[0] < retu["bounArea." + str(b) + ".maxx"] and loca[1] >= retu["bounArea." + str(b) + ".miny"] and loca[1] < retu["bounArea." + str(b) + ".maxy"] and loca[2] >= retu["bounArea." + str(b) + ".minz"] and loca[2] < retu["bounArea." + str(b) + ".maxz"]:
						founArea = True
						area = b
						break
			#if retu["bounAreaCoun"] > 0 and founArea == False:
			if retu["bounAreaCoun"] > 1 and founArea == False:
				area = retu["bounAreaCoun"]

			obje = retu["boun." + str(area) + ".leng"]

			retu.update({"boun." + str(area) + "." + str(obje) + ".iden" : iden})
			retu.update({"boun." + str(area) + "." + str(obje) : name})
			retu.update({"boun." + str(area) + "." + str(obje) + ".x" : loca[0]})
			retu.update({"boun." + str(area) + "." + str(obje) + ".y" : loca[1]})
			retu.update({"boun." + str(area) + "." + str(obje) + ".z" : loca[2]})
			retu.update({"boun." + str(area) + "." + str(obje) + ".stat" : stat})
			retu.update({"boun." + str(area) + "." + str(obje) + ".rang" : rang})
			retu.update({"boun." + str(area) + "." + str(obje) + ".magn" : magn})
			retu.update({"boun." + str(area) + "." + str(obje) + ".minx" : minx})
			retu.update({"boun." + str(area) + "." + str(obje) + ".maxx" : maxx})
			retu.update({"boun." + str(area) + "." + str(obje) + ".miny" : miny})
			retu.update({"boun." + str(area) + "." + str(obje) + ".maxy" : maxy})
			retu.update({"boun." + str(area) + "." + str(obje) + ".minz" : minz})
			retu.update({"boun." + str(area) + "." + str(obje) + ".maxz" : maxz})
			# record the area each object belongs to
			retu.update({"bounObje." + str(iden) + ".area" : area})
			retu.update({"bounObje." + str(iden) + ".inde" : obje})

			retu["boun." + str(area) + ".leng"] += 1

			iden += 1

	retu.update({"bounObjeCoun" : iden})

	
	
	for a in range(retu["bounAreaCoun"]):
		#### sort bounds into a list for each dimension
		boux = []
		bouy = []
		bouz = []
		for b in range(retu["boun." + str(a) + ".leng"]):
			iden = retu["boun." + str(area) + "." + str(b) + ".iden"]
			stat = retu["boun." + str(area) + "." + str(b) + ".stat"]
			rang = retu["boun." + str(area) + "." + str(b) + ".rang"]
			if stat:
				boux.append(["boun." + str(area) + "." + str(b) + ".minx", iden])
				boux.append(["boun." + str(area) + "." + str(b) + ".maxx", iden])
				bouy.append(["boun." + str(area) + "." + str(b) + ".miny", iden])
				bouy.append(["boun." + str(area) + "." + str(b) + ".maxy", iden])
				bouz.append(["boun." + str(area) + "." + str(b) + ".minz", iden])
				bouz.append(["boun." + str(area) + "." + str(b) + ".maxx", iden])
			else:
				x = retu["boun." + str(area) + "." + str(b) + ".x"]
				y = retu["boun." + str(area) + "." + str(b) + ".y"]
				z = retu["boun." + str(area) + "." + str(b) + ".z"]
				if rang:
					magn = retu["boun." + str(area) + "." + str(b) + ".magn"]
					boux.append([x - magn, iden, 0])
					boux.append([x + magn, iden, 1])
					bouy.append([y - magn, iden, 0])
					bouy.append([y + magn, iden, 1])
					bouz.append([z - magn, iden, 0])
					bouz.append([z + magn, iden, 1])
				else:
					boux.append([x, iden])
					bouy.append([y, iden])
					bouz.append([z, iden])
		...
		bouy = sorted(bouy)
		bouz = sorted(bouz)
		# make a list of indices per object
		bounInde = []
		for b in range(retu["boun." + str(a) + ".leng"]):
			bounInde.append([])
		for b in range(len(boux)):
			inde = boux[b][1]
			bounInde[inde].append(b)
		for b in range(len(bouy)):
			inde = bouy[b][1]
			bounInde[inde].append(b)
		for b in range(len(bouz)):
			inde = bouz[b][1]
			bounInde[inde].append(b)
		for b in range(retu["boun." + str(a) + ".leng"]):
			retu.update({"boun." + str(a) + "." + str(b) + ".inx0" : bounInde[b][0]})
			retu.update({"boun." + str(a) + "." + str(b) + ".inx1" : bounInde[b][1]})
			retu.update({"boun." + str(a) + "." + str(b) + ".iny0" : bounInde[b][2]})
			retu.update({"boun." + str(a) + "." + str(b) + ".iny1" : bounInde[b][3]})
			retu.update({"boun." + str(a) + "." + str(b) + ".inz0" : bounInde[b][4]})
			retu.update({"boun." + str(a) + "." + str(b) + ".inz1" : bounInde[b][5]})
		lis_ = ["x", "y", "z"]
		for dime in lis_:
			if dime == "x": boun = boux[:]
			elif dime == "y": boun = bouy[:]
			elif dime == "z": boun = bouz[:]
			retu.update({"boun." + str(a) + ".bou" + dime + "Leng" : len(boun)})
			for b in range(retu["boun." + str(a) + ".bou" + dime + "Leng"]):
				retu.update({"bou" + dime + "." + str(a) + "." + str(b) + ".posi" : boun[b][0]})
				retu.update({"bou" + dime + "." + str(a) + "." + str(b) + ".obje" : boun[b][1]})
				retu.update({"bou" + dime + "." + str(a) + "." + str(b) + ".maxi" : boun[b][2]})
	return retu
...
def PathInit(pathList = [], physList = [], scenObje = "scen_obje"):
	Blen = BlenLink()
	for path in pathList:
		...
		iden = Blen.Objects()[scenObje].game.properties["path"].value
		Blen.Objects()[scenObje].game.properties["path"].value = iden + 1
		#numb = Blen.Pad_(numb)
		Prop(propName = "path", propType = 'BOOL', propValu = path[1])
		#Prop(propName = "numb", propType = 'INT', propValu = int(numb))
		#Prop(propName = "numb", propType = 'INT', propValu = numb)
		Prop(propName = "iden", propType = 'INT', propValu = iden)
		Prop(propName = "stai", propType = 'BOOL', propValu = path[2])
		physList.append(path[0])
	return physList

# add empties that cast rays down and check if theyre on a path
#def PathChec(empt = 1.0, heig = 1.8, forw = (1.0, 0.0), coun = 16):
# TODO: empt
def PathChecSimp(empt = 1.0, heig = 1.8, forw = (1.0, 0.0), coun = 16):
	#import bpy
	import math
	Blen = BlenLink()
	logiDire = LogiDireLink()
	...
	Blen.Pare(name)
	#bpy.ops.transform.translate(value=(math.cos(angl) * empt, math.sin(angl) * empt, heig), constraint_axis=(True, True, True), constraint_orientation='GLOBAL', mirror=False, proportional='DISABLED', proportional_edit_falloff='SMOOTH', proportional_size=1)
...
def PathChec(empt = 1.0, heig = 1.8, forw = (1.0, 0.0), coun = 16):
	...
	# TODO
	#contDict["text"] = logiDire + "path.py"
	contDict["text"] = logiDire + "path_new_.py"
	a = 0
	while a < coun:
		Blen.Empt()
		Blen.Name(name + "." + "path" + "." + Blen.Pad_(a))
...
# Cycl()
def Char(loca = (0.0, 0.0, 0.0), dire = "", cyclArms = True, acti = []):
	Blen = BlenLink()
	Math = MathLink()
	logiDire = LogiDireLink()
	...
	# record the highest vertex location
	name = Blen.Object().name
	Blen.Sele(name + "." + "head")
	...
	# ground cast. add logic to check the location of the ground the character is standing on
	# update: this is now a part of Path()
	#Blen.Sele(name)
	#GrouCast(heig = heig)
	# movement animation
	Blen.Sele(name)
	...
	Prop(propName = "grav", propType = 'FLOAT', propValu = 9.8)
	sens = "PROPERTY"
	sensOpti = LogiSensOpti()
	sensDict = LogiSensDict(typ_ = sens)
	contDict = LogiContDict()
	...
	# TODO: trying new path code
	# path check. add logic to keep a character over a path mesh
	#PathChec(empt = empt, heig = heig, coun = coun)
	Blen.Sele(name)
	Blen.Loca(loca)
...
def GrouCast(heig = 1.8, scenName = ""):
	...
	# TODO: make sure defaults work here like theyre supposed to
...
def Cycl(acti = [], dire = ""):
	...
	for act_ in acti:
		...
		CyclPath(dire = dire, name = act_["acti"])
		Blen.Sele(name)
...
def CyclPath(dire = "", name = "", armsPath = True):
	...
	if armsPath == True:
		part.append("arms")
	for par_ in part:
		expr = BlenCommLink() + " " + "-b" + " " + dire + "cycl_" + name + "_" + par_ + ".blend" + " " + "--python tempScri.py"
		scri = Head()
		scri += "def main():\n"
		scri += "\tBlenGame.CyclWrit()\n"
		scri += "main()\n"
		fileObje = open("tempScri.py", mode = "w")
		fileObje.write(scri)
		fileObje.close()
		os.system(expr)
		line = Pyth.FileTo__Line("data" + os.sep + "cycl")
		#print(line)
...
def CyclWrit():
	...
	if Blen.Object().type == 'CURVE':
		...
		Blen.Conv()
	poinList = []
	for poin in Blen.Vertices():
		poinList.append(str(tuple(bpy.data.objects[path].location + poin.co)))
	Pyth.LineTo__File(poinList, "data" + os.sep + "cycl")
...
def MotiSimp(spee = 0.2):
	...
	actuDict["offset_location"] = (0.0, -spee, 0.0)
	Logi(sensOpti = sensOpti, sensDict = sensDict, sensName = sensDict["key"], contDict = 'linkCont', actuDict = actuDict, actuName = sensDict["key"])
	Prop(propName = "dire", propType = 'INT', propValu = -1)
...
def Cont(faci = (1.0, 0.0), tracHeig = 4.0, grav = -9.8, veloInit = 1.5, scenObje = "scen_obje"):
	import math
	Blen = BlenLink()
	Math = MathLink()
	logiDire = LogiDireLink()
	...
	#vect = (vect[0] * 2.0, vect[1] * 2.0, tracHeig)
	Blen.Loca(Math.VectAdd_(vect, loca))
	Blen.Name(name4)
	Blen.Pare(name + "." + "look")
	Blen.ApplRota()
	...
	loca = Blen.LocaRead()
	Blen.Sele(scenObje)
	PropSet_(propName = "cont", propValu = name)
	Blen.Sele(name + ".look")
	Blen.Loca(loca)
	Blen.Sele(name)
...
def CharCame(cameName = "Camera", tab = False, faci = (1.0, 0.0), scroSens = 10, lookY___Limi = True, lookY___LimiInve = False, lookY___Uppe = 1.710422666954443, lookY___Lowe = 1.2915436464758039, charList = []):
	...
	Prop(propName = "cameMaxi", propType = "INT", propValu = round(1.0 / 0.1))
	#Prop(propName = "cameCurr", propType = "INT", propValu = round(0.3 / 0.1))
	Prop(propName = "cameCurr", propType = "INT", propValu = 6)
	...
	# TODO: var names inconsistent
	LookY___(lookY___Limi = lookY___Limi, lookY___LimiInve = lookY___LimiInve, lookY___LimiUppe = lookY___Uppe, lookY___LimiLowe = lookY___Lowe)
	rotz = math.atan2(faci[1], faci[0])
	rotz -= math.pi / 2.0
	Blen.RotaSet_((80.0, 0.0, math.degrees(rotz)))
	#Blen.Pare(char + "." + "look")
	Blen.Loca(loca = char + "." + "trac" + "." + "posi", worl = True)
	Blen.Pare(char + "." + "trac" + "." + "posi")
	Blen.Sele(char + ".look")
...
def PathPrinAll_(area = 0):
	...
	a = 0
	while a < len(Blen.Vertices()):
		PathPrin(blen = "", meshName = "path_find", area = area, node = a, size = len(Blen.Vertices()))
		a += 1
...
def PathPrin(meshName = "path_mesh", area = 0, node = 0):
	...
	Blen.Sele(meshName)
	size = len(Blen.Vertices())
	vertLoca = (Blen.Vertices()[a].co.x, Blen.Vertices()[a].co.y, Blen.Vertices()[a].co.z)
...
def Ray_Expo(obstList = [], stat = [], room = [], boun = [], bounOnly = [], tria = True):
	...
	Pyth = PythLink()
	Blen = BlenLink()
	Pyth.LineTo__File([], "data" + os.sep + "ray_")
	a = 0
	while a < len(obstList):
		obst = obstList[a]
		Blen.Sele(obst)
		...
		Pyth.LineTo__File([name, str(sta_), str(roo_), str(boun), str(bounOnl_), verb, cenb, norb, polb, vert, cent, norm, poly], "data" + os.sep + "ray_", mode = "a")
		a += 1
...
def Audi(audiList = [], loopList = [], bar_List = [], bpm_ = 120, dire = ""):
	...
	while a < len(audiList):
		...
		if loopList != []:
			if len(loopList) >= a - 1:
				loop = loopList[a]
		# TODO: implement the loading of this sound file
