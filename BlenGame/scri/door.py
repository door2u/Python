...
def main():
	...
	polyList = []
	coun = scen.objects[scen.name + ".door.vari"]["door"]
	for a in range(coun):
		polyList.append(scen.objects[scen.name + ".door.vari"]["door." + str(a)])
	...
	if clic and (pathPoly in polyList):
		scen.objects[scen.name + ".door"].visible = 0
		scen.objects[scen.name + ".unwa"]["unwa.0"] = False
