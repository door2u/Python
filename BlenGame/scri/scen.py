...
def main():
	...
	from scri import Math
	from scri import game
	from scri import move
	from scri import path
	from scri import cycl
	from scri import look_x
	from scri import look_y
	from scri import orie
	from scri import acti
	...
	for a in range(variDict["charCoun"]):
		...
		if char:
			...
			if upda:
				...
				loca, orientat, pathPoly = path.Path(obje.worldPosition, obje.orientation, obje["pathPoly"], moveVect, variDict["inpuMagn"], spee, offs, scen, False, True, False, False, math, mathutils, Math)
				# place the object vertically on the surface of the path
				loca, orientat, pathPoly = path.Path(loca, orientat, pathPoly, moveVect, variDict["inpuMagn"], 0.0, 0.0, scen, True, False, False, False, math, mathutils, Math)

				#loca, orientat, pathPoly = path.Path(obje.worldPosition, obje.orientation, obje["pathPoly"], moveVect, variDict["inpuMagn"], spee, offs, scen, False, True, False, False, math, mathutils, Math)
				#loca, orientat, pathPoly = path.Path(loca, orientat, pathPoly, moveVect, variDict["inpuMagn"], 0.0, 0.0, scen, True, False, True, False, math, mathutils, Math)
...
def Look(charName, lookName, cameName, variDict, scen, look_x, look_y, math, mathutils):
	...
	scen.objects[scen.name + ".came"].orientation = EuleMatr((math.radians(lookY___Angl), orientat[1], orientat[2]), mathutils)
