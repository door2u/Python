...
def Path(posi, orie, pathPoly, moveVect, moveMagn, spee, offs, scen, use_GrouCast, use_PosiAdju, use_Orie, use_OrieCons, math, mathutils, Math):

	# read path geometry
	from scri import path_geom
	vertList, edgeList, polyList, normList, polyEdgeList, edgeNormList, edgeRefeList, bordList, bordVertPolyList, bordEdgeVertList = path_geom.PathGeom()
	...
	while checInte:
		...
		sam1 = Same(ver1, vertList[edgeList[edg1][1]], nor1, loca, pathPoly, pol1, Math)
		sam2 = Same(ver2, vertList[edgeList[edg2][1]], nor2, loca, pathPoly, pol2, Math)
		sam3 = Same(ver3, vertList[edgeList[edg3][1]], nor3, loca, pathPoly, pol3, Math)
		...
		for a in range(len(sameList)):
			...
			if sameList[a] == False and (edg_List[a] in edgeExcl) == False:
				...
				if use_PosiAdju and callPosiAdju:
					loca = PosiAdju(bor_List[a], ver_List[a], nor_List[a], loca, pathPoly != refeList[a], Math)
					checInte = True
					edgeExcl.append(edg_List[a])
					break
...
def Same(ver1, ver2, nor1, loca, pathPoly, pol1, Math):
	retu = False
	samePoly = False
	if pol1 == pathPoly: samePoly = True
	sameSide = Math.Faci(nor1, ver1, loca)
	sam1 = False
	if samePoly == sameSide: sam1 = True
	return sam1

# keep an object on the path
def PosiAdju(bord, vert, norm, loca, inve, Math):
