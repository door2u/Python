...
def main():
	...
	if line == True:
		...
		while i != 0:
			...
			if switch == True:
				# TODO: this was changed to dire
				dir_ = Math.Vect((beginx, beginy, 0.0), (endx, endy, 0.0))
				angl = math.degrees(math.atan2(dir_[1], dir_[0]))
				while angl < 0.0:
					angl += 360.0
	...
	if freq:
		...
		for a in range(rand):
			...
			while True:
				...
				if rand <= cuto:
					...
					break
			# TODO: why 1 / 60. try other divisions
			len_ /= 60.0
			leng.append(len_)
		...
		while i != 0:
			...
			for a in range(len(rota)):
				degrees[a] += rota[a]
			vert_list.append((endx, endy, 0.0))

			tangWidt = 0.1
			tan1 = math.atan2(endy - beginy, endx - beginx) + math.pi / 2.0
			tan2 = tan1 + math.pi
			tan1 = (tangWidt / 2.0 * math.cos(tan1), tangWidt / 2.0 * math.sin(tan1))
			tan2 = (tangWidt / 2.0 * math.cos(tan2), tangWidt / 2.0 * math.sin(tan2))

			vert_list.append((endx + tan1[0], endy + tan1[1], 0.0))
			vert_list.append((endx + tan2[0], endy + tan2[1], 0.0))

			# center line
			edgeList.append((len(vert_list) - 3, len(vert_list) - 6))
			# inside offset line
			edgeList.append((len(vert_list) - 2, len(vert_list) - 5))
			# edge connections center to inside
			edgeList.append((len(vert_list) - 2, len(vert_list) - 3))
			# outside offset line
			edgeList.append((len(vert_list) - 1, len(vert_list) - 4))
			# edge connections center to outside
			edgeList.append((len(vert_list) - 1, len(vert_list) - 3))

			polyList.append([len(vert_list) - 6, len(vert_list) - 3, len(vert_list) - 2, len(vert_list) - 5])
			polyList.append([len(vert_list) - 3, len(vert_list) - 6, len(vert_list) - 4, len(vert_list) - 1])
		...
		edgeList.append((1079, 2))
		edgeList.append((1078, 1))
		edgeList.append((1077, 0))

		polyList.append([1079, 2, 0, 1077])
		polyList.append([1077, 0, 1, 1078])

		Blen.Uplo([vert_list, edgeList, polyList])

		# TODO: position camera
		Blen.Sele("Camera")
		loca = Blen.LocaRead()
		Blen.Loca((lowestx + (highestx - lowestx) / 2.0, lowesty + (highesty - lowesty) / 2.0, loca[2]))

		stri = []
		a = 0
		for a in range(len(rota)):
			stri.append("rota.append(" + str(rota[a]) + ")")
			stri.append("leng.append(" + str(leng[a]) + ")")

		# TODO: stri to para
		# TODO: os.sep
		global dire
		global numb
		Pyth.LineTo__File(stri, dire + "out_/spir_" + numb + "_para")
