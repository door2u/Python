...
def main():
	...
	#print(grou)
	#grou = [9, 4, 8, 1, 0, 5, 6, 7, 2, 6, 7, 3]

	####
	#Pyth.LineTo__File([], "face/seam/seam")
	####
	...
	for a in range(len(grou)):
		...
		sepa = Pyth.FileTo__Line("face/sepa_seam.py")
		scri += "\t"
		for line in sepa:
			scri += line + "\n"
		scri += "\n"
		...
		fileObje = open("tempExpo.py", mode = "w")
		fileObje.write(scri)
		fileObje.close()

		syst = Modu.blenComm + " -b " + dire + "face_" + Pyth.Pad_(grou[a], 4) + ".blend --python tempExpo.py"
		os.system(syst)
