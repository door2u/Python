...
class Scri():
	...
	def Init(self):
		...
		# apply a script to a single file
			...
			save = False
		# read the last file in the out_ directory, don't create a new file unless the script does
		elif self.mode == 4:
			self.out_ = self.geneCont + "out_" + os.sep
			self.vari.update({"dire" : self.geneCont})
			self.vari.update({"name" : self.name})
...
def Dist():
	...
	retu.mode = 4
	retu.name = "dist"
	retu.dire = "flor" + os.sep + "dist" + os.sep
	retu.scri = "flor" + os.sep + "dist" + os.sep + "tree_dist"
	retu.geneCont = geneCont + "flor" + os.sep + "dist" + os.sep
	retu.save = False
	retu.Init()
	return retu
...
def Spir():
	...
	retu = Scri()
	retu.name = "spir"
	retu.dire = "spir" + os.sep
	retu.scri = "spir" + os.sep + "spir"
	retu.geneCont = geneCont + "spir" + os.sep
	retu.Init()
	return retu
...
def Text():
	...
	retu = Scri()
	retu.name = "text"
	retu.dire = "text" + os.sep
	retu.scri = "text" + os.sep + "text"
	retu.geneCont = geneCont + "text" + os.sep
	retu.Init()
	# TODO
	retu.vari.update({"nudg" : 1.0})
	return retu
...
def TextNudg():
	...
	#retu.name = "text_nudg"
	retu.name = "text"
	#retu.dire = "text_nudg" + os.sep
	retu.dire = "text" + os.sep
	retu.geneCont = geneCont + "text" + os.sep
	#retu.scri = "text_nudg" + os.sep + "text_nudg"
	retu.scri = "text" + os.sep + "text"
	retu.Init()
	# TODO
	#retu.pref = "text"
	retu.vari.update({"nudg" : 0.3})
	return retu
...
def Make(scriList = [], fileCoun = 1, incr = True, rendImag = True, rendVide = False, framCoun = 1, exe_ = True):
	...
	# cap at 9999
	if fileCoun > 9999: fileCoun = 9999
	# read iden file
	if incr: iden = IdenRead()
	else: iden = {}
	# pick - pick the next library file to modify. when all have been picked, start over.
	pick = []
	for a in range(len(scriList)): pick.append(0)
	brea = 0
	true = True
	while true:
		for a in range(len(scriList)):
			...
			if mode == 3:
				paraOut_ = ""
			# TODO: make a script to create the first file for dist
			if mode == 4:
				#print("here")
				# TODO?
				paraOut_ = ""
				if fil_Coun > 0:
					...
					#	fil_ = fileList[b].split(".")
					#	if len(fil_) > 0  and fil_[len(fil_) - 1] == "blend1":
					#		syst = "mv " + out_ + fileList[b] + " ~/Documents/trash/"
					#		os.system(syst)
					# pick the last file in the out_ directory
					syst = blenComm + " -b " + geneCont
					vari.update({"blenPath" : out_ + name + "_" + numb + ".blend"})
					vari.update({"numb" : numb})
					fil_ = Conc(syst, pref)
					syst += "out_" + os.sep
					syst = Conc(syst, pref, els_ = name)
					syst += Pyth.Pad_(fil_Coun - 1, 4) + ".blend" + " --python tempScri.py"
				else:
					print("error: Make() function called on a script with mode 4 which requires at least one blend file in the out_ content directory. exiting.")
					return 0
			if paraOut_ != "":
				#print("here")
			...
			if rendImag or rendVide:
				...
				if mode == 1:
					...
					fil_ += numb
				# TODO: or mode == 4. need a skinned tree to render first. or maybe skin like in original tree script. or maybe add skinning to old script.
				if mode == 2:
					fil_ = geneCont + "out_" + os.sep + name + "_" + numb
				if mode == 3:
