...
def Symm():
	import bpy
	poin = len(bpy.context.object.data.splines[0].bezier_points)
	pair = []
	a = poin
	for a in range(1, int(poin / 2)):
		pair.append([a, poin - a])
	edge = [0, int(poin / 2)]
	for pai_ in pair:
		bpy.context.object.data.splines[0].bezier_points[pai_[1]].co = bpy.context.object.data.splines[0].bezier_points[pai_[0]].co
		bpy.context.object.data.splines[0].bezier_points[pai_[1]].co.y = -1.0 * bpy.context.object.data.splines[0].bezier_points[pai_[1]].co.y
		bpy.context.object.data.splines[0].bezier_points[pai_[1]].handle_left = bpy.context.object.data.splines[0].bezier_points[pai_[0]].handle_right
		bpy.context.object.data.splines[0].bezier_points[pai_[1]].handle_left.y = -1.0 * bpy.context.object.data.splines[0].bezier_points[pai_[1]].handle_left.y
		bpy.context.object.data.splines[0].bezier_points[pai_[1]].handle_right = bpy.context.object.data.splines[0].bezier_points[pai_[0]].handle_left
		bpy.context.object.data.splines[0].bezier_points[pai_[1]].handle_right.y = -1.0 * bpy.context.object.data.splines[0].bezier_points[pai_[1]].handle_right.y
	y = bpy.context.object.data.splines[0].bezier_points[edge[0]].handle_right.y
	bpy.context.object.data.splines[0].bezier_points[edge[0]].handle_right = bpy.context.object.data.splines[0].bezier_points[edge[0]].co
	bpy.context.object.data.splines[0].bezier_points[edge[0]].handle_left = bpy.context.object.data.splines[0].bezier_points[edge[0]].co
	bpy.context.object.data.splines[0].bezier_points[edge[0]].handle_right.y = y
	bpy.context.object.data.splines[0].bezier_points[edge[0]].handle_left.y = -1.0 * y
	y = bpy.context.object.data.splines[0].bezier_points[edge[1]].handle_left.y
	bpy.context.object.data.splines[0].bezier_points[edge[1]].handle_left = bpy.context.object.data.splines[0].bezier_points[edge[1]].co
	bpy.context.object.data.splines[0].bezier_points[edge[1]].handle_right = bpy.context.object.data.splines[0].bezier_points[edge[1]].co
	bpy.context.object.data.splines[0].bezier_points[edge[1]].handle_left.y = y
	bpy.context.object.data.splines[0].bezier_points[edge[1]].handle_right.y = -1.0 * y

def CurvRand(vari = 0.01):
	import bpy
	poin = len(bpy.context.object.data.splines[0].bezier_points)
	poin = int(poin / 2)
	bpy.context.object.data.splines[0].bezier_points[0].handle_right.y += Math.RandRang(-vari, vari)
	for a in range(1, poin):
		bpy.context.object.data.splines[0].bezier_points[a].co.x += Math.RandRang(-vari, vari)
		bpy.context.object.data.splines[0].bezier_points[a].co.y += Math.RandRang(-vari, vari)
		bpy.context.object.data.splines[0].bezier_points[a].co.z += Math.RandRang(-vari, vari)
		bpy.context.object.data.splines[0].bezier_points[a].handle_left.x += Math.RandRang(-vari, vari)
		bpy.context.object.data.splines[0].bezier_points[a].handle_left.y += Math.RandRang(-vari, vari)
		bpy.context.object.data.splines[0].bezier_points[a].handle_left.z += Math.RandRang(-vari, vari)
	bpy.context.object.data.splines[0].bezier_points[poin].co.x += Math.RandRang(-vari, vari)
	bpy.context.object.data.splines[0].bezier_points[poin].co.z += Math.RandRang(-vari, vari)
	bpy.context.object.data.splines[0].bezier_points[poin].handle_left.y += Math.RandRang(-vari, vari)

def main():

	# TODO:
	# for this and fern, might want to scale by distance from edge
	...
	if peta:
		...
		# randomize points
		random.seed()
		rand = random.randint(0, maxiPoin)
		for a in range(rand):
			CurvRand()
		Symm()
		...
		for a in range(1, int(leng / 2)):
			edgeList.append((a, leng - a))
		...
		Blen.Sele("obje")
		Blen.Name("flow")
		Blen.MateSet_("Material")
		Blen.Edit()
		bpy.ops.mesh.edge_face_add()
		bpy.ops.mesh.faces_shade_smooth()
		Blen.Edit()
		# pick number of petals
		...
		random.seed()
		rand = random.randint(miniPeta, maxiPeta)
		bpy.context.object.modifiers['Array'].count = rand
		z = 360.0 / rand
		Blen.Sele("Empty")
		...
		# material
		bpy.data.node_groups["NodeGroup"].nodes['Mapping'].scale[0] = Math.RandRang(1.0, 10.0)
		bpy.data.node_groups["NodeGroup"].nodes['Mapping'].scale[1] = Math.RandRang(1.0, 10.0)
		bpy.data.node_groups["NodeGroup"].nodes['Noise Texture'].inputs[1].default_value = Math.RandRang(1.0, 10.0)
		bpy.data.node_groups["NodeGroup"].nodes['Noise Texture'].inputs[2].default_value = Math.RandRang(0.0, 16.0)
		bpy.data.node_groups["NodeGroup"].nodes['Noise Texture'].inputs[3].default_value = Math.RandRang(0.0, 10.0)
		bpy.data.node_groups["NodeGroup"].nodes['Hue Saturation Value'].inputs[0].default_value = Math.RandRang(0.0, 1.0)
		bpy.data.node_groups["NodeGroup"].nodes['Hue Saturation Value'].inputs[1].default_value = Math.RandRang(0.0, 2.0)
		bpy.data.node_groups["NodeGroup"].nodes['Hue Saturation Value'].inputs[2].default_value = Math.RandRang(0.0, 2.0)
		bpy.data.node_groups["NodeGroup"].nodes['Mix'].inputs[1].default_value[0] = Math.RandRang(0.0, 1.0)
		bpy.data.node_groups["NodeGroup"].nodes['Mix'].inputs[1].default_value[1] = Math.RandRang(0.0, 1.0)
		bpy.data.node_groups["NodeGroup"].nodes['Mix'].inputs[1].default_value[2] = Math.RandRang(0.0, 1.0)
		bpy.data.node_groups["NodeGroup"].nodes['Mix'].inputs[2].default_value[0] = Math.RandRang(0.0, 1.0)
		bpy.data.node_groups["NodeGroup"].nodes['Mix'].inputs[2].default_value[1] = Math.RandRang(0.0, 1.0)
		bpy.data.node_groups["NodeGroup"].nodes['Mix'].inputs[2].default_value[2] = Math.RandRang(0.0, 1.0)
		bpy.data.node_groups["NodeGroup"].nodes['Mix.001'].inputs[0].default_value = Math.RandRang(0.0, 1.0)
		bpy.data.node_groups["NodeGroup"].nodes['Bright/Contrast'].inputs[1].default_value = Math.RandRang(0.5, 1.0)
	...
	if run_:
		...
		try:
			paraDict = para
			exte = True
			direOut = dire + "out_" + os.sep + "flow_" + numb + "_para"
		except:
			# TODO: where else did this get messed up?
			# TODO: call this file to list?
			paraDict = Pyth.FileTo__Line("flor/flow/flow_para")
		...
		while curx < maxx:
			...
			while cury < maxy:
				...
				if in__:
					...
					#loca = bpy.context.object.data.splines[0].bezier_points[1].co
					last = tuple(bpy.context.object.data.splines[0].bezier_points[1].co)
					#print(last)
					#return 0
					...
					for a in range(len(vertList)):
						...
						if rand > 0.8:
							...
							if rand >= 0.9:
								...
								if rand < 0.5:
									Blen.Scal((1.0, 1.0, -1.0))
								Blen.RotaSet_((0.0, 180.0 - vertList[a][1], 0.0 + 180.0))
								Blen.Rota((0.0, 0.0, z), loca = True)
								Blen.Join("leaf.001")
							else:
								Blen.Sele("leaf")
					...
					#Blen.Sele("stem." + Blen.Pad_(emptCoun))
					#loca = bpy.context.object.data.vertices[len(bpy.context.object.data.vertices) - 1].co

					#Blen.Sele("Empty")
					#Blen.Loca((last[0] + x, last[1] + y, last[2]))
					...
					#print(last)
					Blen.Loca((last[0] + x, last[1] + y, last[2]))
					Blen.RotaSet_((0.0, vertList[len(vertList) - 1][1], 0.0))
					scal = Math.RandRang(0.25, 1.0)
					Blen.Scal((scal, scal, scal))
					#vertList.append([prog, angl])
					Blen.Join("stem." + Blen.Pad_(emptCoun))
					Blen.Name("stem." + Blen.Pad_(emptCoun))
					...
					bpy.context.object.rotation_euler[2] = z + math.pi
