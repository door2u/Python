...
def Clic():
	...
	sensOpti = LogiSensOpti()
	sensDict = LogiSensDict(typ_ = sens)
	contDict = LogiContDict(typ_ = "LOGIC_AND")
...
def PathGeom():
	...
	vertNorm = Math.VertNorm(len(vertList), polyList, normList)
	...
	for a in range(len(polyEdgeList)):
		for b in range(len(polyEdgeList[a])):
			...
			if edgeNormList[edge] == []:
				...
				edgeNormList[edge] = [edgeNorm, a]

	# add geometry to a script
	scriStri = "\n"
	scriStri += "def PathGeom():\n"
	scriStri += "\tvertList = ["
	for a in range(len(vertList)):
		scriStri += "(" + str(vertList[a][0]) + ", " + str(vertList[a][1]) + ", " + str(vertList[a][2]) + ")"
		if a < len(vertList) - 1:
			scriStri += ", "
	scriStri += "]\n"
	scriStri += "\tedgeList = ["
	for a in range(len(edgeList)):
		scriStri += "[" + str(edgeList[a][0]) + ", " + str(edgeList[a][1]) + "]"
		if a < len(edgeList) - 1:
			scriStri += ", "
	scriStri += "]\n"
	scriStri += "\tpolyList = ["
	for a in range(len(polyList)):
		scriStri += "[" + str(polyList[a][0]) + ", " + str(polyList[a][1]) + ", " + str(polyList[a][2]) + "]"
		if a < len(polyList) - 1:
			scriStri += ", "
	scriStri += "]\n"
	scriStri += "\tnormList = ["
	for a in range(len(normList)):
		scriStri += "(" + str(normList[a][0]) + ", " + str(normList[a][1]) + ", " + str(normList[a][2]) + ")"
		if a < len(normList) - 1:
			scriStri += ", "
	scriStri += "]\n"
	scriStri += "\tpolyEdgeList = ["
	for a in range(len(polyEdgeList)):
		scriStri += "[" + str(polyEdgeList[a][0]) + ", " + str(polyEdgeList[a][1]) + ", " + str(polyEdgeList[a][2]) + "]"
		if a < len(polyEdgeList) - 1:
			scriStri += ", "
	scriStri += "]\n"
	scriStri += "\tedgeNormList = ["
	for a in range(len(edgeNormList)):
		scriStri += "(" + str(edgeNormList[a][0][0]) + ", " + str(edgeNormList[a][0][1]) + ", " + str(edgeNormList[a][0][2]) + ")"
		if a < len(edgeNormList) - 1:
			scriStri += ", "
	scriStri += "]\n"
	scriStri += "\tedgeRefeList = ["
	for a in range(len(edgeNormList)):
		scriStri += str(edgeNormList[a][1])
		if a < len(edgeNormList) - 1:
			scriStri += ", "
	scriStri += "]\n"
	scriStri += "\tbordList = ["
	for a in range(len(bordList)):
		bord = [bordList[a][0]]
		if len(bordList[a]) == 2:
			bord.append(bordList[a][1])
		else:
			bord.append(-1)
		scriStri += "[" + str(bord[0]) + ", " + str(bord[1]) + "]"
		if a < len(bordList) - 1:
			scriStri += ", "
	scriStri += "]\n"
	scriStri += "\treturn vertList, edgeList, polyList, normList, polyEdgeList, edgeNormList, edgeRefeList, bordList\n"
	return scriStri
...
def PathUnwa(pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, markList, scenName, hideList):
	...
	for mark in markList:
		Blen.Sele(variObje)
		Prop(propName = "unwaStar." + str(unwaSet_), propType = 'INT', propValu = unwaCoun)
		polyList = PolyMark(pathObje, minxLis1, minxInd1, maxxLis1, maxxInd1, minyLis1, minyInd1, maxyLis1, maxyInd1, mark)
		Blen.Sele(variObje)
		for b in range(len(polyList)):
			Prop(propName = "unwa." + str(unwaCoun), propType = 'INT', propValu = polyList[b])
			unwaCoun += 1
		Prop(propName = "unwaEnd_." + str(unwaSet_), propType = 'INT', propValu = unwaCoun)
